import { DIMENSION_FORM, equals, isBlank, isNumberForm, isString, isUndeclared } from "./format.util";
import { ArrayElementType, ElementType, NN, Range, Relation } from "src/type/index.type";
import { PathString } from "./typeorm.util";

export const clone = <T>(obj: T):T => {
  const val = JSON.parse(JSON.stringify(obj)) as T;
  if(typeof val === 'string' )
    return  (val as unknown as string).replace(/^\"/g, '') as unknown as T
  return val;
}
...
export const propertiesForEach = (obj: any, predict: (key: string|number, value: any, wrapper: any) => any) => {
  if( !obj ) return;
...
export const timeElapse = () => new TimeElapse()
